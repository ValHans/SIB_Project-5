...
DB_URI = f"postgresql+pyscopg2://{config('MB_DB_USER')}:{config('MB_DB_PASS')}@{config('MB_DB_HOST')}:{str(config('MB_DB_PORT'))}/{config('MB_DB_DBNAME')}"
app = Flask(__name__)
...
def user():
    if request.method == 'GET':
        ...
        request = [{"id": u.id, "name": u.name, "city": u.city, "telp": u.telp} for u in users]
